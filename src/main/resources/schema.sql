...
(
    ...
    `MEETUP_LAT`     DOUBLE        NOT NULL,
    `MEETUP_LON`     DOUBLE        NOT NULL,
    `CREATED_AT`     TIMESTAMP     NOT NULL,
    `UPDATED_AT`     TIMESTAMP     NULL
);
...
(
    ...
    `REQUESTER_ID` BIGINT    ,
    `AGENT_ID`     BIGINT    ,
    `CHAT_ROOM_STATUS`  INTEGER   NOT NULL DEFAULT 2,
    `CREATED_AT`   TIMESTAMP NOT NULL
);
...
(
    ...
    `STATUS`              VARCHAR(255) NOT NULL DEFAULT 'READY' COMMENT 'OK, READY, CANCEL',
...
ALTER TABLE
    `PAY`
    ADD CONSTRAINT `pay_site_user_id_foreign`
        FOREIGN KEY (`USER_ID`) REFERENCES `SITE_USER` (`ID`)
            ON DELETE CASCADE;
...
ALTER TABLE
    `PAY`
    ADD CONSTRAINT `pay_order_id_foreign`
...
ALTER TABLE
    ...
    ADD CONSTRAINT `pay_cancel_detail_payment_id_foreign`
